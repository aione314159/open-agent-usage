...
색은 항상 소비량을 따릅니다. 30% 이하는 초록, 60% 이하는 파랑, 80% 이하는 주황, 80% 초과는 빨강입니다. 실행 중이지만 숫자를 줄 수 없는 에이전트(브리지를 설치하기 전의 Claude Code 등)는 자리를 지키며 회색 `--`를 표시합니다. 따라서 막대에서 아이콘이 사라지는 것은 그 에이전트가 멈췄다는 뜻뿐입니다.
