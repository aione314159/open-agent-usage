...
**Claude Code · Codex · Gemini · Antigravity 사용량을 macOS 메뉴 막대에.**

[![버전](https://img.shields.io/github/v/release/aione314159/open-agent-usage?label=%EB%B2%84%EC%A0%84&color=8957e5)](../../releases/latest)
[![다운로드](https://img.shields.io/github/downloads/aione314159/open-agent-usage/total?label=%EB%8B%A4%EC%9A%B4%EB%A1%9C%EB%93%9C&color=2ea043)](../../releases)
[![macOS 15+](https://img.shields.io/badge/macOS-15%2B-000000?logo=apple&logoColor=white)](#시스템-요구-사항)
[![Apple silicon + Intel](https://img.shields.io/badge/Apple%20silicon%20%2B%20Intel-%EC%9C%A0%EB%8B%88%EB%B2%84%EC%84%A4-555555)](#시스템-요구-사항)
[![Apple 공증](https://img.shields.io/badge/Apple%20%EA%B3%B5%EC%A6%9D-Developer%20ID-0a7ea4)](#설치)
[![라이선스](https://img.shields.io/badge/%EB%9D%BC%EC%9D%B4%EC%84%A0%EC%8A%A4-%EB%8F%85%EC%A0%90%20%EC%86%8C%ED%94%84%ED%8A%B8%EC%9B%A8%EC%96%B4-9e9e9e)](LICENSE)
...
[📥 다운로드](../../releases/latest) · [💡 기능](#주요-기능) · [⚙️ 설정](#추가-설정) · [🔄 계정 전환](#claude-code-계정-전환) · [🐞 문제 신고](../../issues/new)
...
평소에는 이렇게 보입니다. 작은 막대에 서비스마다 아이콘과 퍼센트가 나란히 표시됩니다. 위 화면에서는 에이전트 세 개가 실행 중입니다. 노치가 없는 Mac에서는 메뉴 막대에, 노치가 있는 Mac에서는 노치에 이어지는 다이내믹 아일랜드로 표시되며 설정에서 직접 고를 수 있습니다.

1. **Claude Code** — 5시간 창. 작업을 가장 먼저 멈추는 한도입니다. 7일 창은 패널에 있습니다.
2. **Codex** — 현재 창의 사용량.
3. **Antigravity** — Gemini 모델 할당량. 서드파티 모델 할당량은 패널에 있습니다.

수치는 기본적으로 사용한 비율을 보여주며, 설정에서 한 번에 남은 양 표시로 바꿀 수 있습니다. 어느 쪽이든 색은 항상 소비량을 따릅니다. 30% 이하는 초록, 60% 이하는 파랑, 80% 이하는 주황, 80% 초과는 빨강입니다. 실행 중이지만 숫자를 줄 수 없는 에이전트(브리지를 설치하기 전의 Claude Code 등)는 자리를 지키며 회색 `--`를 표시합니다. 따라서 막대에서 아이콘이 사라지는 것은 그 에이전트가 멈췄다는 뜻뿐입니다.

## 주요 기능

- **노치든 메뉴 막대든, Mac에 맞게** — 노치가 있는 Mac에서는 노치에 이어지는 다이내믹 아일랜드로, 노치가 없는 Mac에서는 메뉴 막대 항목으로 표시됩니다. 자동으로 판별하며 설정에서 지정할 수도 있습니다.
- **4개 에이전트를 한 줄에서 확인** — Claude Code, Codex, Gemini, Antigravity의 사용량을 나란히 표시하므로, 확인할 때마다 앱 4개를 따로 열 필요가 없습니다.
- **세션을 클릭하면 바로 전환** — 패널에는 최근 Claude Code 세션이 프로젝트, 브랜치, 모델, 작업 내용과 함께 나열되며, 클릭하면 해당 창으로 바로 전환됩니다.
...
- **Claude Code 계정을 클릭 한 번으로 전환** — 개인용과 업무용 구독을 나란히 두고 로그아웃 없이 오갈 수 있습니다. 인증 메일을 기다릴 일이 없습니다. (Pro)
...
개인용과 업무용, Claude 구독을 두 개 갖고 있다면 이 과정이 익숙할 겁니다. 한쪽을 다 쓰면 로그아웃, 다른 쪽으로 로그인, 인증 메일을 기다리고, 링크를 클릭. 매번 말이죠.

![Claude Code 카드, 계정 버튼에 번호를 매긴 모습](docs/account-switch-annotated.png)

이메일은 의도적으로 가렸습니다. Claude Code 카드에서:

1. **저장되어 있지만 지금 쓰고 있지 않은 계정** — 클릭하면 전환됩니다. 아래에 로그인이 얼마나 남았는지 표시됩니다.
2. **사용 중인 계정** — 체크가 붙고 눌리지 않습니다. 3일 남으면 주황, 만료되면 빨강으로 바뀌며 잠기므로 로그인이 조용히 끊기지 않습니다.
3. **도움말** — 계정을 추가하는 방법. 명령은 바로 복사할 수 있습니다.

패널에서는 한 번의 클릭으로 끝납니다. 그럴 수 있는 이유는 한 번도 로그아웃하지 않기 때문입니다.

로그아웃은 리프레시 토큰까지 함께 지웁니다. 다음 로그인에서 인증 메일이 또 필요한 것은 그 때문입니다. 인증 정보를 복사해 두었다가 나중에 다시 써 넣으면 아무것도 취소되지 않습니다. 전환해서 떠난 계정은 돌아왔을 때도 여전히 로그인 상태입니다. 리프레시 토큰이 살아 있는 한, 약 4주 동안. 게다가 Claude Code 가 쓸 때마다 갱신되므로 가끔 돌아오는 계정이라면 만료되지 않습니다.

저장한 계정은 Claude Code 카드 위의 버튼이 됩니다. 사용 중인 것은 체크가 붙고 눌리지 않으며, 각 버튼에는 리프레시 토큰이 얼마나 남았는지 표시됩니다. 3일 남으면 주황, 만료되면 빨강으로 바뀌며 잠깁니다. 리프레시 토큰이 조용히 만료되는 것이야말로 이 기능이 막으려는 일이기 때문입니다. 이미 열어 둔 `claude` 는 스스로 따라오므로 다시 시작할 것이 없습니다.

계정 추가는 계정마다 한 번, 터미널에서 명령 한 줄로 끝납니다. 그 명령은 앱에 함께 들어 있고 도움말 창에서 설치할 수 있습니다. 그다음부터 전환은 버튼뿐입니다.

인증 정보는 본인의 Mac 에, 본인만 읽을 수 있는 형태로 남습니다. `~/.claude.json` 은 쓸 때마다 백업됩니다. 아무것도 업로드되지 않습니다.

## 사용량을 어떻게 읽는가

네 개의 에이전트, 네 가지 완전히 다른 방식. 어느 것도 네트워크로 묻지 않습니다. 모든 숫자가 이 Mac 에서 나옵니다.

![사용량을 읽는 방식](docs/quota-sources.ko.png)

- **Claude Code** 는 사용량을 상태 표시줄 명령에 넘길 뿐 어디에도 저장하지 않습니다. bridge 스크립트가 필요한 이유입니다. 그 명령을 감싸 사본을 남기고, 원래 출력은 그대로 통과시킵니다.
- **Codex** 는 스스로 기록을 남기므로 `~/.codex` 에서 바로 읽습니다.
- **Gemini** 는 읽을 파일 자체가 없습니다. 수치는 macOS 손쉬운 사용을 통해 자신의 창에서 가져옵니다. 그 권한이 필요한 이유입니다.
- **Antigravity** 는 실행할 때마다 바뀌는 로컬 포트로 응답하므로, 먼저 포트를 찾고 프로세스 이름과 대조한 뒤에 요청을 보냅니다.
...
| 세션 목록 (각 Claude Code 세션이 무엇을 하고 있는지 확인하고 클릭 한 번으로 해당 창으로 이동) | 지원 | 지원 |
| Claude Code 계정 전환 (로그아웃 불필요, 인증 메일 불필요) | 미지원 | 지원 |
...
3. 두 번 클릭하면 바로 열립니다. 이 빌드는 Developer ID 인증서로 서명되고 Apple 공증을 받았기 때문에 Gatekeeper가 경고 없이 통과시킵니다. 우회 절차는 필요 없습니다.
...
**Language**는 즉시 적용되며 재시작이 필요 없습니다. **Theme**는 기본적으로 macOS를 따르지만, **Light** 또는 **Dark**로 고정할 수도 있습니다. **Text size**(Small/Medium/Large)는 막대와 이 설정 창을 함께 확대·축소합니다. **수치 표시**는 네 서비스를 한 번에 사용량 표시와 남은 양 표시로 바꿉니다. 색은 그대로 소비량을 따르므로 빨강은 여전히 거의 소진을 뜻합니다.
...
**Show the quota strip**은 화면 상단에 수치를 표시합니다. 노치에 이어 붙일 수도 있고 메뉴 막대 항목으로 둘 수도 있으며, 클릭하면 상세 정보를 볼 수 있습니다. **Where to show it**은 이 둘 중 하나를 고르는 항목입니다. 다이내믹 아일랜드는 내장 디스플레이의 노치에 이어지고, 메뉴 막대는 같은 수치를 상태 항목으로 보여 줍니다. 기본값은 다이내믹 아일랜드이며, 노치가 없는 디스플레이에서는 이어 붙일 홈이 없으므로 자동으로 메뉴 막대로 넘어갑니다. **Expand on hover**는 클릭뿐 아니라 마우스를 올려도 상세 정보가 열리도록 합니다. 기본값은 꺼짐입니다. 포인터가 노치를 지날 때마다 시트가 열리면 실수로 트리거되기 쉽기 때문입니다. **Show time until reset**은 각 퍼센트 뒤에 초기화까지 남은 시간을 덧붙입니다. 예: `51% 1h59m`.
...
페이지 상단에는 현재 라이선스 상태가 표시되며, 체험 기간 중에는 남은 일수도 함께 표시됩니다. 그 아래 **Free and Pro** 표에는 어떤 기능이 무료(사용량 막대)이고 어떤 기능이 Pro 전용(Claude Code 계정 전환, `open-agent-usage` 명령줄 인터페이스)인지 정리되어 있습니다. 체험 기간이 끝나면 Pro로 표시된 항목만 작동을 멈춥니다.
