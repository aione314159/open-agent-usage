...
- **一鍵切換 Claude Code 帳號** — 個人與公司兩組訂閱並存，切換時不必登出，也就不必再等一次驗證信。（Pro）
...
如果你同時有個人與公司兩組 Claude 訂閱，這個流程你應該很熟：一邊用完了，登出，登入另一組，等驗證信，點連結。每次都來一遍。

![Claude Code 卡片，帳號按鈕已標號](docs/account-switch-annotated.png)

信箱是刻意打上馬賽克的。在 Claude Code 卡片上：

1. **已存起來、但目前沒在用的帳號** — 點一下就切過去。下方寫著這組登入還剩多久。
2. **使用中的帳號** — 打勾且點不動。剩三天內轉橘，過期轉紅並停用，所以登入不會無聲失效。
3. **說明** — 怎麼新增帳號，附可直接複製的指令。

面板上點一下就換好了，而它做得到的原因是——它從來不登出。

登出會把 refresh token 跟其他東西一起刪掉，這正是下次登入還要再收一次驗證信的原因。把憑證複製起來、之後再寫回去，什麼都不會被撤銷：你切走的那個帳號，等你切回來時仍然是登入狀態，只要它的 refresh token 還活著。那大約是四週，而且 Claude Code 每次用到就會往後延，所以只要偶爾切回去，那組帳號就一直切得動。

每個存起來的帳號都是 Claude Code 卡片上的一顆按鈕。使用中的那顆打勾且點不動，每顆按鈕都寫著它的 refresh token 還剩多久——剩三天轉橘，過期轉紅並停用，因為 refresh token 悄悄過期正是這個功能要防的事。已經開著的 `claude` 會自己跟上，不必重開任何東西。

新增帳號每個帳號做一次，在終端機用一行指令完成，那個指令隨 App 一起發布，可以從說明視窗一鍵安裝。之後切換就只是按鈕而已。

你的憑證留在自己的 Mac 上，只有你讀得到，而且 `~/.claude.json` 在每次寫入前都會先備份。不會上傳任何東西。

## 用量是怎麼讀來的

四個 agent，四種完全不同的機制。沒有一個是透過網路問來的——每個數字都來自這台 Mac。

![用量怎麼讀來的](docs/quota-sources.zhHant.png)

- **Claude Code** 把配額交給狀態列指令後就不再保存，這正是需要 bridge 腳本的原因：它包在那個指令外面，留下一份副本，再把原本的輸出原封不動往下傳。
- **Codex** 自己會留紀錄，所以直接從 `~/.codex` 讀。
- **Gemini** 根本沒有檔案可讀。數字是透過 macOS 輔助使用從它自己的視窗上取得的，這也是為什麼要那個權限。
- **Antigravity** 在一個每次啟動都會變的本機埠上回答，所以要先找出埠、確認是它的程序，才送出任何請求。

完整的樣子，包含帳號切換：

![架構](docs/architecture.zhHant.png)
